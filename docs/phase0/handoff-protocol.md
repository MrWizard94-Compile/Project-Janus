...
4. **Grok** runs `aether patch submit -f patch.json` (dry-run validate).
5. On failure, kernel returns structured errors per layer; Grok revises and resubmits.
6. On success, **Grok** runs `aether patch submit -f patch.json --apply` or `aether patch apply -f patch.json` using the stored receipt.
7. **Claude** reviews the resulting diff in the worktree branch and accepts or rejects.
